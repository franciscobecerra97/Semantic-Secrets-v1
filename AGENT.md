# AGENT.md — Semantic Secrets research contract (v3.3 calibration amendment)
...
- P9-v3A completed on 2026-08-25 as architecture and preregistration only. Its v3.0.0 files remain the immutable initial freeze. P9-v3A.1 completed the prospective pipeline/support audit; P9-v3A.2 prospectively superseded only the historical human-ground-truth method; and P9-v3A.3 prospectively freezes the previously missing development-threshold calibration rule. The active P9-v3B contract composes v3.0 through v3.3; no v3 model, weight, capability image, inference, fitted threshold, policy, authentication, or cryptographic experiment has run.
- P9-v3B requires explicit authorisation and new `cap-v3-*` data. It has no human participants or annotators. Each final image must have a project-authored closed-label reference specification; manifest, scenarios, image hashes, and deterministic opportunities must be audited and frozen before perception. V3.3 resolves the threshold-method blocker prospectively: fixed 0.01-grid, development-only, staged entity-first component calibration with exact per-stratum ranking, fallback, score provenance, offline replay, and validation isolation. Count/geometry never fit thresholds; no task or pipeline pooling is allowed. The remaining data blocker is the actual project-authored scenario/prompt content and resulting ground-truth freeze. Preparation code and local tests are engineering evidence only; they did not start P9-v3B or pass V3-A1. The locked image must rerun every compiler case. P9-v3C requires constructive V3-A1 and separate preregistration/data; P10 requires constructive V3-A2.
...
P9-v3A.1 fixed the prospective pipeline/resource/support design, P9-v3A.2 supplied project-authored model-blind ground truth, and P9-v3A.3 now freezes executable development calibration before output. P9-v3B still requires explicit instruction, complete project-authored scenario/prompt inputs, and a hash-bound `ground-truth-freeze-v3.2.0` before inference. After permitted score capture, the only authorised order is entity fit → frozen scopes → downstream scores → offline fit → integrated development replay → `threshold-freeze-v3.3.0` → formal authorization → validation/repeat. See `docs/formal_specification_v3_2.md`, `docs/formal_specification_v3_3.md`, and decisions `P9-v3A.2-00`/`P9-v3A.3-00`.
