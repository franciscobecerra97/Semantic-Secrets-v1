...
The obsolete annotation-resource blocker is resolved methodologically by v3.2; the actual project-authored scenario/prompt content, capability data, and ground-truth freeze do not yet exist. The frozen text also does not define the development-threshold optimization objective or tie rule, so execution must not select one after viewing outputs. No capability image, weight, model output, or experimental result was created during preparation.
