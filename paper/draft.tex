...
\begin{abstract}
Most authentication requires exact secret reproduction or exact cryptographic possession. This planning manuscript studies a different primitive: approximate semantic reconstruction, in which independently reconstructed prompts and images may yield compatible canonical semantic credentials. The resulting fuzzy predicate creates a complete acceptance region; legitimate variation must be accepted without making realistic or AI-assisted semantic alternatives too easy to reach. Even a discriminative predicate raises a separate privacy problem if its graph, policy, record, or transcripts enable inversion, offline candidate validation, adaptive probing, or cross-domain linkage. We therefore ask whether reconstructable visual semantics can be stable and discriminative, whether the induced region survives finite-budget attacks, and whether the frozen predicate can be privately verified. This is not a completed system or a human-memory study. P8 narrowed the prospective C1--C4 contribution after finding prior art for the component mechanisms. P9-v2 then tested two frozen local extractors that each had to infer a scene and author complete exact credential JSON; each produced an invalid graph on its first formal validation fixture. One invalid among 32 caps best-case schema validity at $31/32=96.875\%$, below the preregistered 98\% requirement. Gate V2-A failed and full semantic accuracy was not estimated. P9-v3A preregistered a modular observation/compiler architecture, and a prospective v3.1 audit selected EGTR as the graph-native comparator while freezing resource and support rules before output. No v3 weight, capability image, inference, authentication, or cryptographic experiment has run. P10 remains blocked.
\end{abstract}
...
\section{Reconstruction media are not credentials}
...
If all gates pass, C1 would establish technical reconstructability of the scoped free-language generative path; C2 would establish only the measured incremental value of the frozen system-derived typed policy; C3 would characterise the accepted region under finite-budget attackers; and C4 would establish construction-specific private and unlinkable verification. C1/C2 are enabling contributions; C3/C4 carry the prospective principal security/privacy contribution. Their scientific meanings are unchanged by the narrative reframe. P8 rejected C5 as standalone and retained adaptive semantic verification-oracle leakage as mandatory evidence inside C3/C4.

The frozen v2 path stopped at the first risk: neither tested extractor met the structured-output prerequisite. P9-v3A provides an independently justified representation architecture but no empirical rescue. Whether either modular pipeline works remains unresolved, as do reconstruction, policy, guessing-region, private-construction, and client-side practicality questions. A research GPU does not establish deployment feasibility. The controlled methodology cannot establish human memorability, recall, usability, preference, accessibility, natural secret-selection entropy, or real-world authentication time; nor can it establish superiority to passwords, passkeys, biometrics, or hardware authenticators. A future participant study would require separate ethics, design, and evidence.

\section{Conclusion}

Semantic Secrets asks what follows when authentication moves from exact reproduction to approximate semantic reconstruction. Visual generation is a testable reconstruction/normalisation boundary, not a source of entropy or an asserted memory aid. The induced acceptance region is the central security object, and privacy is a separate construction-specific property rather than a consequence of hashing semantics. P9-v2 falsified its frozen monolithic extractor path, and that evidence remains unchanged. P9-v3A supplies the explicit reframe; v3.1 prospectively freezes the final two-pipeline and support design before output. It passes no empirical gate. P9-v3B is the next permitted experiment only after explicit authorisation and resolution of the annotation blocker; P9-v3C, P10, attacker evaluation, and cryptographic prototyping remain blocked by their gates.
