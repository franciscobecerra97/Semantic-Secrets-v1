...
\begin{abstract}
This planning manuscript defines a revised research direction; it does not report a completed authentication system. We ask whether a remembered visual concept can be reconstructed through independently generated images, converted locally into a canonical typed semantic graph, and privately verified under a system-derived policy. P8 narrowed the prospective contribution after finding prior art for the component mechanisms. P9-v2 then tested two frozen local extractors that each had to infer a scene and author the complete exact credential JSON. Each produced an invalid graph on its first formal validation fixture. With 32 validation fixtures, one invalid output caps best-case schema validity at $31/32=96.875\%$, below the preregistered 98\% requirement. Gate V2-A failed; full semantic accuracy was not estimated. P9-v3A preregistered a distinct modular architecture: perception models emit bounded observations and provenance, while a deterministic compiler emits a canonical graph or typed failure. A prospective v3.1 audit then selected EGTR as the graph-native comparator, separated GPU capacity from measured consumption, and froze feasible type-support rules before output. No v3 weight, capability image, inference, authentication, or cryptographic experiment has run. P10 remains blocked.
\end{abstract}
...
\section{P9-v3 modular extraction preregistration}
...
This work treats visual generation as a testable reconstruction layer, not a source of magical entropy, and privacy as a construction-specific property rather than a consequence of hashing semantics. P9-v2 falsified its frozen monolithic extractor path, and that evidence remains unchanged. P9-v3A supplies the explicit reframe; v3.1 prospectively freezes the final two-pipeline and support design before output. It passes no empirical gate. P9-v3B is the next permitted experiment only after explicit authorisation and resolution of the annotation blocker; P9-v3C, P10, attacker evaluation, and cryptographic prototyping remain blocked by their gates.
